...
export type StockCardProps = {
  symbol: string;
  name: string;
  price?: number; // Make props optional to reflect reality
  change?: number;
...
const StockCard = ({ symbol, name, price, change, changePercent, onClick }: CardComponentProps) => {
  // Use a logical OR (||) to provide 0 as a fallback if the value is missing.
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="text-sm text-gray-400 truncate">{name || 'Name not available'}</div>
        </div>
        {isPositive ? <ArrowUpRight className="w-8 h-8 text-green-500" /> : <ArrowDownRight className="w-8 h-8 text-red-500" />}
      </div>

      <div className="mt-6">
        {/* Here is the fix: (price || 0) ensures .toFixed() is always called on a number. */}
